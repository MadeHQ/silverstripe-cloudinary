window.tmpl.cache['ss-cloudinary-uploadfield-downloadtemplate'] = tmpl(
    '{% for (var i=0, files=o.files, l=files.length, file=files[0]; i<l; file=files[++i]) { %}' +
        '<li id="{%=file.field_id%}-holder" class="ss-uploadfield-item template-download{% if (file.error) { %} ui-state-error{% } %}" data-fileid="{%=file.id%}">' +
            ...
            '<div class="ss-uploadfield-item-info">' +
                '{% if (!file.error) { %}' +
                    ...
                    '<input id="{%=file.field_id%}" class="text file-attach-field" name="{%=file.fieldname%}__URL" type="text" value="{%=file.url%}"/>'+
                '{% } %}' +
                '{% if (file.error) { %}' +
