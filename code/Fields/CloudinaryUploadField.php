...
class CloudinaryUploadField extends UploadField
{
    ...
    public function Field($properties = array()) {
		...
		Requirements::javascript('cloudinary/javascript/CloudinaryUploadField_downloadtemplate.js');
	...
	public function upload(SS_HTTPRequest $request) {
		...
		$arrData = array(
			...
			'Format'			=> CloudinaryUtils::file_format($arrUploaderDetails['public_id']),
		);

		$file = new CloudinaryFile($arrData);
		$file->write();
	...
	public function fileattach() {
		if (isset($_POST['SourceURL'])) {
			...
			$publicID = CloudinaryUtils::file_name($sourceURL);
			$file = CloudinaryFile::get()->filter('URL', $sourceURL)->first();

			if(!$file){
				$api = CloudinaryUtils::api();
				$resource = $api->resource($publicID, array("resource_type" => "raw"));
				$arrResource = $resource->getArrayCopy();

				$file = new CloudinaryFile(array(
					'FileTitle' => $publicID,
					'URL'		=> $arrResource['url'],
					'FileSize'	=> $arrResource['bytes'],
					'Format'	=> CloudinaryUtils::file_format($publicID),
				));
				$file->write();
			}

			if ($file) {
				$this->value = $iVideoID = $file->ID;
                ...
                if($bManyMany) {
                    ...
                    DB::query('
                        UPDATE "' . $baseDataClass . '"
                            SET "LastEdited" = \'' . date('Y-m-d H:i:s', strtotime(SS_Datetime::now()->getValue())) . '\'' . '
                            WHERE "ID" = '. $iID
                    );
                }
	...
	protected function customiseCloudinaryFile(CloudinaryFile $file) {
		$file = $file->customise(array(
			'UploadFieldThumbnailURL' => $file->Icon(),
